...
- 認証は **Google OAuth 2.0**。**既存 Fitbit OAuth トークンは移行不可** → 再同意必須
  - ⚠ 2026-05 訂正: この「再同意必須」は **GH API へ移行する時だけ**の話。**アカウント統合(§2)自体は Fitbit Web API のトークンを無効化しない**(Fitbit 公式フォーラムが明言)。詳細は末尾「ローンチ後アップデート(2026-05-31)」
...
設計前調査(2026-04)から約 5 週間。その間に**消費者向け Google Health アプリが正式ローンチ**(Fitbit アプリの後継)し、開発者向け Google Health API も breaking change を重ねながら前進した。多角 Web 調査(6 角度 fan-out → 重要主張を独立検証、34/35 を confirmed)を行い、設計前提との差分を確定した。**この節は 2026-04 の調査本文を上書きせず、差分レイヤとして追記している**(記事化のため経緯を残す)。

## A. 結論: いま緊急対応は不要

- **Fitbit Web API(`api.fitbit.com`)は 2026-05-31 時点でまだ稼働中**。`get_profile` / `list_devices` 相当が正常応答。9 月停止は不変、ローンチで前倒し・延期なし
...
|---|---|---|
| 既存トークン移行不可→再同意(§1, §3) | ⚠ 半分誤り | 「再同意必須」は **GH API へ移行する時だけ**。**アカウント統合(§2)は Fitbit Web API トークンを無効化しない**(Fitbit 公式フォーラム明言) |
