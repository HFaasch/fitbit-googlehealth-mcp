...
| 日 | 当日の運動ログ | Fitbit 生 body | intraday dataset 点数 |
|---|---|---|---|
| A | なし | 6699 B | 88 |
| B | あり | 2528 B | 23 |
| C | あり | 900 B | **0(`activities-heart-intraday` キー自体なし)** |
| D | なし | 2928 B | 30 |
...
1. **Fitbit Web API はまだ生きている**。`get_profile` が正常応答。9 月停止は不変なので **急ぐ必要はない**
2. **アカウント統合は Fitbit Web API トークンを無効化しない**。これが設計時の最大の懸念だったが空振り — Fitbit 公式フォーラムがアカウント統合はトークンを無効化しないと明言。「再同意必須」は GH API へ移行する時だけの話で、統合と混同していた(`research.md §1, §3` に訂正を注記)
